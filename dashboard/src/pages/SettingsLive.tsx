...
const captionDefaults: CaptionSettings = {
  font: 'Arial',
  font_size: 18,
  color: '&H00FFFFFF',
...
function readCaptionSettings(raw: Record<string, unknown>): CaptionSettings {
  return {
    ...captionDefaults,
    ...raw,
    font_size: Number(raw.font_size ?? captionDefaults.font_size),
    outline_width: Number(raw.outline_width ?? captionDefaults.outline_width),
    shadow: Number(raw.shadow ?? captionDefaults.shadow),
    max_words_per_line: Number(raw.max_words_per_line ?? captionDefaults.max_words_per_line),
    bold: Boolean(raw.bold ?? captionDefaults.bold),
  } as CaptionSettings;
}

export function Settings({ user }: Props) {
  const [chatId, setChatId] = useState('');
  const [captionSettings, setCaptionSettings] = useState<CaptionSettings>(captionDefaults);
  ...
  useEffect(() => {
    api.settings()
      ...
      .catch(() => setChatId(''));
    api.captionSettings()
      .then((settings) => setCaptionSettings(readCaptionSettings(settings)))
      .catch(() => setCaptionSettings(captionDefaults));
  }, []);

  const updateCaption = <K extends keyof CaptionSettings>(key: K, value: CaptionSettings[K]) => {
    setCaptionSettings((current) => ({ ...current, [key]: value }));
  };
  ...
  const saveCaptions = async (event: FormEvent) => {
    ...
    try {
      await api.updateCaptionSettings(captionSettings);
      showToast({ tone: 'success', message: 'Caption settings saved' });
    } catch (error) {
      showToast({ tone: 'error', message: error instanceof Error ? `Failed: ${error.message}` : 'Failed to save captions' });
    } finally {
  ...
  const primaryColor = assToHex(captionSettings.color);
  const highlightColor = assToHex(captionSettings.highlight_color);
  const outlineColor = assToHex(captionSettings.outline_color);
      ...
      <section className="settingsPanel captionSection">
        ...
        <form onSubmit={saveCaptions}>
          <div className="captionGrid">
            <label>
              Font
              <select value={captionSettings.font} onChange={(event) => updateCaption('font', event.target.value)}>
                {fonts.map((font) => <option key={font} value={font}>{font}</option>)}
              </select>
            </label>
            <label>
              Font Size: {captionSettings.font_size}px
              <input
                type="range"
                min="12"
                max="72"
                value={captionSettings.font_size}
                onChange={(event) => updateCaption('font_size', Number(event.target.value))}
              />
            </label>
            <label>
              Position
              <select
                ...
                onChange={(event) => updateCaption('position', event.target.value as CaptionSettings['position'])}
              >
            ...
            <label>
              ...
              <select
                ...
                onChange={(event) => updateCaption('style', event.target.value as CaptionSettings['style'])}
              >
            ...
            <label>
              Primary Color
              <input type="color" value={primaryColor} onChange={(event) => updateCaption('color', hexToAss(event.target.value))} />
            </label>
            <label>
              Highlight Color
              <input
                ...
                onChange={(event) => updateCaption('highlight_color', hexToAss(event.target.value))}
              />
            </label>
            <label>
              Boundary Color
              <input
                type="color"
                value={outlineColor}
                onChange={(event) => updateCaption('outline_color', hexToAss(event.target.value))}
              />
            </label>
            <label>
              Boundary: {captionSettings.outline_width}px
              <input
                type="range"
                min="0"
                max="8"
                value={captionSettings.outline_width}
                onChange={(event) => updateCaption('outline_width', Number(event.target.value))}
              />
            </label>
            <label>
              Shadow: {captionSettings.shadow}px
              <input
                type="range"
                min="0"
                max="5"
                value={captionSettings.shadow}
                onChange={(event) => updateCaption('shadow', Number(event.target.value))}
              />
            </label>
            <label>
              Words per line: {captionSettings.max_words_per_line}
              <input
                type="range"
                min="1"
                max="8"
                value={captionSettings.max_words_per_line}
                onChange={(event) => updateCaption('max_words_per_line', Number(event.target.value))}
              />
            </label>
            <label className="captionToggle">
              <input
                ...
                onChange={(event) => updateCaption('bold', event.target.checked)}
              />
              Bold letters
            </label>
          </div>
          <div className={`captionPreview captionPreview-${captionSettings.position}`}>
            <span className="captionPreviewSwatch" style={{ backgroundColor: highlightColor }} />
            <span
              className="captionPreviewText"
              style={{
                color: primaryColor,
                fontFamily: captionSettings.font,
                fontSize: `${captionSettings.font_size}px`,
                fontWeight: captionSettings.bold ? 800 : 500,
                WebkitTextStroke: `${captionSettings.outline_width}px ${outlineColor}`,
                textShadow: captionSettings.shadow ? `0 ${captionSettings.shadow}px ${captionSettings.shadow * 2}px #000` : 'none',
              }}
            >
              caption preview
            </span>
