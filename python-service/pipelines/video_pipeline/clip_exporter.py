...
MIN_EXPORTED_CLIP_DURATION_SEC = 4.0
...
def export_clips(
    ...
    for i, moment in enumerate(moments, start=1):
        clip_duration = float(moment.get("clip_end", 0.0)) - float(moment.get("clip_start", 0.0))
        if clip_duration < MIN_EXPORTED_CLIP_DURATION_SEC:
            continue
        ...
        try:
            ...
            probe = ffmpeg_utils.probe_video(clip_path)
            if float(probe.get("duration", 0.0)) < MIN_EXPORTED_CLIP_DURATION_SEC:
